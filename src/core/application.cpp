...
void Application::showTrayMenu() {
  ...
  if (cmd == 1) {
   toggleOverlayVisible();
    ...
    if (!_overlay_visible && !_settings_visible) toggleOverlayVisible();
...
void Application::toggleOverlayVisible() {
...
LRESULT CALLBACK Application::WndProc(HWND hwnd, UINT msg, WPARAM w_param, LPARAM l_param) {
...
bool Application::createApplication(HINSTANCE& h_instance) {
  ...
  // Create handle
  _wc = {sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0, 0,
    h_instance,
    _h_icon, // big icon
